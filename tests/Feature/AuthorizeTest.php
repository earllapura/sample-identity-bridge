...
class AuthorizeTest extends TestCase
{
    ...
    {
        $client = Mockery::mock(OAuthLink::class);
        $client->shouldReceive('getClientInfo')->with('test_app')->andReturn((object) [
            'client'     => json_decode('
                {
                    "client_id":"test_app", "id":"12121212121",
        ...
        $client->shouldReceive('getScopeInfo')->with('email')->andReturn((object) [
            'scopes'      => json_decode('
                [{
                    "name":"email",
                    "description":"Grants permission to read your email address"
                }]'),
            'statusCode' => 200,
        ]);
        $this->app->instance(OAuthLinkInterface::class, $client);
